...
const STATIC_FUELS = [
  ...
  { id: 'e20', name: 'E20',            color: '#f97316', glow: 'rgba(249,115,22,0.4)', badge: 'ประหยัด' },
  { id: 'e85', name: 'E85',            color: '#0d9488', glow: 'rgba(13,148,136,0.4)', badge: 'พลังงานสะอาด' },
...
export default function OrderPage({ fuels: liveFuels, onNavigate }) {
  ...
  return (
    <div className="min-h-screen bg-[#07090E] text-white flex items-center justify-center p-4 sm:p-8 font-sans">
      
      {/* ── KIOSK SCREEN HARDWARE WRAPPER ── */}
      <div 
        className="w-full max-w-6xl rounded-[2rem] p-1 sm:p-3 shadow-2xl relative"
        style={{ 
                ...
                {KIOSK_FUELS.map(fuel => {
                  ...
                  return (
                    <motion.button // Ensure min width for mobile horizontal scroll
                      ...
                      className={`relative min-w-[240px] md:min-w-0 md:w-full snap-center rounded-2xl p-4 md:p-5 text-left transition-all duration-300 border-2 overflow-hidden flex-shrink-0 ${
                         isActive ? 'bg-[#151c28]' : 'bg-[#0f1522] border-transparent hover:border-gray-700'
                      }`}
                      style={{ 
            ...
            <div className="flex-1 p-4 md:p-8 flex flex-col relative md:overflow-y-auto hidden-scrollbar">
                  ...
                  >
                    ...
                    <div className="bg-[#05070a] rounded-3xl p-4 md:p-6 border border-gray-800 shadow-inner flex flex-col justify-between mb-6 md:mb-8"
                      ...
                      <div className="flex justify-between items-end">
                        <div className="w-1/2">
                          ...
                          <p className="text-4xl md:text-6xl lg:text-7xl font-mono font-bold tracking-tighter" style={{ color: inputMode === 'baht' ? '#fff' : selectedFuel.color }}>
                            {inputMode === 'baht' ? (amount || '0') : calculatedBaht.toFixed(2)}
                            <span className="text-lg md:text-2xl ml-1 md:ml-2 font-sans opacity-50">฿</span>
                          </p>
                        </div>
                        <div className="w-px h-12 md:h-16 bg-gray-800 mx-2 md:mx-6" />
                        <div className="w-1/2 text-right">
                          <p className="text-[10px] md:text-sm font-bold text-gray-500 uppercase tracking-widest">{inputMode === 'liter' ? 'จำนวนลิตร (Liters)' : 'ปริมาณน้ำมัน (Liters)'}</p>
                          <p className="text-4xl md:text-6xl lg:text-7xl font-mono font-bold tracking-tighter" style={{ color: inputMode === 'liter' ? '#fff' : selectedFuel.color }}>
                    ...
                    <motion.div className="mt-4 md:mt-8 flex gap-3 md:gap-4 h-16 md:h-20 shrink-0">
                      ...
                      <button 
                        ...
                        className={`flex-1 rounded-xl md:rounded-2xl font-black text-lg md:text-2xl tracking-wide uppercase transition-all duration-300 shadow-xl ${
                          numericAmount > 0 
                  ...
                  >
                      ...
                      <div className="flex bg-[#0f1522] rounded-2xl p-1.5 border border-gray-800 mb-8">
                        <button 
                          ...
                          className={`flex-1 py-4 rounded-xl text-lg font-bold transition-all flex items-center justify-center gap-2 ${
                            deliveryMode === 'delivery' ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-gray-500 hover:text-gray-300'
                          }`}>
                          🛵 เดลิเวอรี่ (Delivery)
                        </button>
                        <button 
                          ...
                          className={`flex-1 py-4 rounded-xl text-lg font-bold transition-all flex items-center justify-center gap-2 ${
                            deliveryMode === 'walkin' ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20' : 'text-gray-500 hover:text-gray-300'
                          }`}>
                          ⛽ เติมที่สถานี (Walk-in)
                        </button>
                      </div>
                    ...
                    <motion.div className="flex gap-3 md:gap-4 h-16 md:h-20 shrink-0">
                      ...
                      <button 
                        ...
                        className={`flex-1 rounded-xl md:rounded-2xl font-black text-sm md:text-2xl tracking-wide uppercase transition-all duration-300 shadow-xl flex items-center justify-center gap-2 md:gap-3 ${
                          (deliveryMode === 'walkin' || location.trim() !== '')
              ...
              <AnimatePresence>
                {isDispensing && (
                  <motion.div
                    initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    className="absolute inset-0 bg-[#05070a]/90 backdrop-blur-xl z-50 flex flex-col items-center justify-center md:rounded-r-3xl rounded-3xl border border-gray-800 p-4 text-center"
                  >
                    <motion.div 
              ...
              <AnimatePresence>
                {showSlip && (
                  <motion.div
                    initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    className="absolute inset-0 bg-[#05070a]/80 backdrop-blur-md z-40 flex items-center justify-center p-4 md:rounded-r-3xl rounded-3xl"
                  >
                    <motion.div 
