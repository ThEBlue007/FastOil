...
// ── Hero Section (รูปไรเดอร์ใหญ่จุใจ) ─────────────────────────────────────────
        ...
        <section className="relative overflow-hidden pt-14 pb-20 px-4"
            ...
            <div className="relative max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
                ...
                <motion.div initial="hidden" animate="show" variants={stagger}>
                    ...
                    <motion.h1 variants={fadeUp} className="text-4xl md:text-5xl xl:text-6xl font-black text-blue-950 leading-tight mb-5">
                        เติมน้ำมัน<br />
                ...
                {/* ── Right: Rider Image Only (ขยายใหญ่) ── */}
                <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
                    transition={{ duration: 0.8, type: "spring", bounce: 0.4 }}
                    className="flex justify-center lg:justify-end relative mt-10 lg:mt-0">
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <div className="w-80 h-80 bg-amber-200/40 rounded-full blur-3xl" />
                    </div>
                    {/* ปรับ w-96 h-96 ให้ภาพใหญ่สะใจ */}
                    <img src={riderImg} alt="FastOil Delivery Rider"
                        className="relative z-10 w-80 h-80 md:w-96 md:h-96 xl:w-[450px] xl:h-[450px] object-contain hover:scale-105 transition-transform duration-500 cursor-pointer"
...
// ── Page Export ───────────────────────────────────────────────────────────────
export default function Home({ fuels, loading, lastUpdate }) {
    // ลบ State และ useEffect เกี่ยวกับ activeOrder ออกให้หมด เพราะไปทำที่ App.jsx แทนแล้ว
    return (
